...
line_rci_alert.py – モチポヨ方式 RCI 監視 & LINE 通知
  * Render では loop_forever() を webhook_receiver から起動
...
def rci(s: pd.Series, n: int) -> float:
    ...
    if len(s) < n:
        return np.nan
    closes = s.tail(n).reset_index(drop=True)         # ← 必ず n 本だけ
    price_rank = closes.rank(method="first").to_numpy(dtype=float)
...
def one_shot(cfg: dict) -> None:
    for name, ticker in cfg["pairs"].items():
        ...
        # --- 本数不足ならスキップ ---
        if len(df) < 52:
            print(f"{name}: まだ {len(df)} 本（最低 52 本必要）");  continue
